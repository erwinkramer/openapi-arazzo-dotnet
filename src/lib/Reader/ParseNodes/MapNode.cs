...
namespace BinkyLabs.OpenApi.Arazzo.Reader
{
    ...
    internal class MapNode : ParseNode, IEnumerable<PropertyNode>
    {
            ...
            context, node)
        {
            ...
            {
                throw new ArazzoReaderException("Expected map.", Context);
            }
        ...
        {
            var jsonMap = _node ?? throw new ArazzoReaderException($"Expected map while parsing {typeof(T).Name}", Context);
            ...
            var nodes = jsonMap.Select(n =>
            {
                ...
                try
                {
                    ...
                    JsonArray arrayNode = n.Value is JsonArray value
                        ? value
                        : throw new ArazzoReaderException($"Expected array while parsing {typeof(T).Name}", Context);
        ...
        public string? GetScalarValue(ValueNode key)
        {
            ...
            if (keyValue is not null)
            {
                var scalarNode = _node[keyValue] is JsonValue jsonValue
                        ? jsonValue
                        : throw new ArazzoReaderException($"Expected scalar while parsing {key.GetScalarValue()}", Context);
